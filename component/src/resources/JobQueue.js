...
export default class JobQueue extends databases.local.RenderJob {
	...
	static async submitJob(job, cb) {
		...
		if (cb) {
			...
			const workerId = RenderWorkers.getAvailableWorkerId();

			if (workerId) {
				await databases.local.RenderWorker.publish(`${workerId}/queue`, job);
			} else {
				await databases.local.RenderJob.patch(job.id, { priority: 2 });
				parentPort.postMessage({ type: 'job_queue/status', status: 'queued' });
			}
		}
	}
