 ...
 */
export const render = async ({ url, headers, deviceType, acceptLanguage, waitForResponse = false, priority = 2 }) => {
	return new Promise((resolve, reject) => {
		if (waitForResponse) {
			JobQueue.submitJob(
				{ url, deviceType, source: 'request', priority, acceptLanguage, headers: JSON.stringify(headers) },
				(error, result) => {
					if (error) {
						reject(error);
					} else {
			...
			JobQueue.submitJob({ url, deviceType, acceptLanguage, priority, headers: JSON.stringify(headers) })
				.then(resolve)
				.catch(reject);
		}
	});
